...
import { Plus, X, Target, Building, ArrowLeft } from 'lucide-react';
...
export default function TopicAddPage() {
  ...
  const initialValues = {
    project_id: useSearchParams()[0].get('project') ?? '',
    keywords: '',
    article_type_id: '',
  ...
  useEffect(() => {
    const fetchArticleTypes = async () => {
      try {
        const response = await PromptTypeService.getPromptTypes();
        if (response.status) {
      ...
      <main className="container mx-auto px-4 py-6 max-w-4xl">
        <div className="max-w-4xl mx-auto">
          <PageHeader
            ...
            description="Set up your content topic with AI-powered insights"
          />
            ...
            onSubmit={async (values, { setSubmitting }) => {
              try {
                ...
                if (response.status) {
                  ...
                  navigate(`/topics`); // Or navigate to the new topic if the response provides an ID
                } else {
                  toast({
                    title: 'Error',
                            ...
                            <div className="mt-4 p-4 border rounded-lg bg-muted/30">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {recommendedKeywords
                                  ...
                                  .map((keyword, index) => (
                                    <Badge
                                      key={`recommended-${keyword.keyword}-${index}`}
                                      variant="outline"
